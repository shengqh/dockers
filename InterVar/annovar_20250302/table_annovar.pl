...
our $REVISION = '$Revision: 8f9c9f42b094bb89b47888b2fddd9824db5e384b $';
our $DATE =	'$Date: 2022-08-02 22:48:58 -0400 (Tue,  2 Aug 2022) $';
	...
	while (<IN>) {
		...
		if (m/^##/) {
			...
			for my $i (0 .. @name-1) {
				...
				} elsif ($name[$i] =~ m/^(1000g\d+|esp\d+|cg\d+|popfreq|nci\d+|ExAC|gnomAD)/i) {
...
sub geneOperation {
	...
	while (<FUNCTION>)
	{
		...
		chop $spliceanno if $gene =~ s/\(([^)]+)\)//g;	#get rid of the trailing ;
		
		#$spliceanno =~ tr/,/;/;													#commented out 20210118, this is actually a bad idea because some times a variant can be both upstream and downstream, and upstream for multiple genes
		#$gene =~ tr/,/;/;																#commented out 20210118, same as above.
		...
		if ($intronhgvs) {
			...
			if ($min_distance > 2) {		#logic is changed 20201211: chr3:49405953 is both exonic and splicing (<5bp away from exon/intron boundary, if -intronhgvs 5 is set), but it should not be changed to intron
				if ($function eq 'splicing') {
					$function = 'intron';
				}
			}
		}
...
=head1 SYNOPSIS
 ...
 Version: $Date: 2022-08-02 22:48:58 -0400 (Tue,  2 Aug 2022) $

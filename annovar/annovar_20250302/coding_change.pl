...
our $DATE =	'$Date: 2025-03-02 22:36:45 -0500 (Sun,  2 Mar 2025) $';  
...
my (%mrnastart, %mrnaend, %name_chr);	#key is transcript name, value is the translation start position in this transcript
while (<GENE>) {
	s/[\r\n]+$//;
	my @field = split (/\t/, $_);
	...
	if ($name_chr{$name}) {		#20250302: already found this name in the file earlier
		if ($chr =~ m/_alt$/) {	#if current chromosome ends with alt then use the previously encountered one (sometimes a transcript is mapped to multiple chromosomes and we have to judge what to do with it; generally we use the last encounter which overwrites earlier encounter
			next;
		}
	}
	$name_chr{$name} = $chr;
...
while (<FASTA>) {
	...
	if (m/^>([\w\.]+)/) {
		...
		if (m/with discordant length/) {	#for example, a transcript can map to both chr7 and chr7_KI270803v1_alt, and we want to use the first one
			if (m/leftmost exon at \S+_alt/) {
				$curstatus = 1;
			}
		}
	} else {
...
=head1 SYNOPSIS
 ...
 Version: $Date: 2025-03-02 22:36:45 -0500 (Sun,  2 Mar 2025) $
